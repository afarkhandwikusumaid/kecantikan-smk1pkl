        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-1 font-sans">
            {navItems.map((item) => (
              <button
                ...
                className={`px-5 py-2 text-sm font-medium transition-all duration-300 rounded-full relative group cursor-pointer ${
          ...
          <div className="hidden lg:flex items-center space-x-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-pink-800 text-white hover:bg-pink-900 font-medium text-sm px-6 py-2.5 rounded-full shadow-sm hover:shadow transition-all duration-300 flex items-center space-x-1"
            >
          ...
          <div className="flex md:hidden items-center">
            <button
              ...
              className="p-2 rounded-lg text-gray-600 hover:text-pink-800 hover:bg-pink-50 focus:outline-none"
          ...
          <div className="px-5 pt-4 pb-8 space-y-2 max-h-[85vh] overflow-y-auto">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleNavClick(item.id)}
                className={`w-full text-left block px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                  activeSection === item.id
                    ? 'text-pink-800 bg-pink-50 font-semibold'
