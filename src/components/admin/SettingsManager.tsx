...
const defaultContact: ContactSettings = {
  address: 'Jl. Sriwijaya No.7, Pekalongan, Jawa Tengah 51111',
  phone: '(0285) 423-456',
  email: 'kecantikan@smk1pekalongan.sch.id',
  mapsUrl: '',
};

const defaultSocial: SocialSettings = {
  instagram: '@smk1pkl_kecantikan',
  facebook: 'SMK 1 Pekalongan Kecantikan',
  youtube: '',
  tiktok: '',
  website: 'https://smk1pekalongan.sch.id',
};
...
export default function SettingsManager() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (contactDoc.exists()) {
        setContact(contactDoc.data() as ContactSettings);
      }
      
      const socialDoc = await getDoc(doc(db, 'settings', 'social'));
      if (socialDoc.exists()) {
        setSocial(socialDoc.data() as SocialSettings);
      }
    } catch (e) {
      console.error('Error fetching settings:', e);
      setErrorMsg('Gagal memuat pengaturan dari database.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveContact = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg('Gagal menyimpan kontak. Periksa rules Firestore.');
    }
  };
  ...
  const handleSaveSocial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg('Gagal menyimpan sosmed. Periksa rules Firestore.');
    }
  };
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #fce7f3, #fdf2f8)' }}>
                <Phone className="w-4.5 h-4.5 text-pink-600" />
              </div>
              <div>
            ...
            <form onSubmit={handleSaveContact} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input type="text" value={contact.mapsUrl}
            ...
            <form onSubmit={handleSaveSocial} className="p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input type="text" value={social.instagram}
                ...
                <div>
                  ...
                  <input type="text" value={social.facebook}
                ...
                <div>
                  ...
                  <input type="text" value={social.youtube}
                ...
                <div>
                  ...
                  <input type="text" value={social.tiktok}
                ...
                <div className="sm:col-span-2">
                  ...
                  <input type="url" value={social.website}
                    onChange={(e) => setSocial({ ...social, website: e.target.value })}
                    className="w-full rounded-xl border border-slate-200 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-300"
                    placeholder="https://smk1pekalongan.sch.id" />
                </div>
              </div>
              <div className="flex justify-end">
                <button type="submit"
